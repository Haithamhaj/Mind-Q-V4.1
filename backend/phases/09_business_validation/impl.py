...
def run(run_id: str, inputs: Mapping[str, Any], config: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
    ...
    if what_if:
        ...
        # Also write as YAML (expected output)
        try:
            import yaml
            what_if_yaml_data = {
                "run_id": run_id,
            ...
            with (out_dir / "what_if.yaml").open("w", encoding="utf-8") as f:
                yaml.dump(what_if_yaml_data, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
        except ImportError:
            # Fallback: write as JSON if yaml not available
            io.write_json_sorted(what_if_yaml_data, out_dir / "what_if.yaml")
    else:
        # Create empty what_if.yaml even if no scenarios
        try:
            import yaml
            empty_what_if = {
                "run_id": run_id,
                "generated_at": datetime.now(TZ).isoformat(),
                "scenarios": [],
                "note": "No what-if scenarios configured for this run",
            }
            with (out_dir / "what_if.yaml").open("w", encoding="utf-8") as f:
                yaml.dump(empty_what_if, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
        except ImportError:
            io.write_json_sorted({
                "run_id": run_id,
                "generated_at": datetime.now(TZ).isoformat(),
    ...
    # Generate summary.json (executive summary for sign-off)
    sla_results = sla_summary_payload.get("results", []) if isinstance(sla_summary_payload, dict) else []
    summary_payload_data = {
        "run_id": run_id,
        "generated_at": datetime.now(TZ).isoformat(),
        "gate_status": gate_status,
        "gate_reasons": gate_reasons,
        "sla_summary": {
            "total_terms": len(sla_results),
            "pass": sum(1 for item in sla_results if isinstance(item, dict) and item.get("status") == "pass"),
            "warn": sum(1 for item in sla_results if isinstance(item, dict) and item.get("status") == "warn"),
            "fail": sum(1 for item in sla_results if isinstance(item, dict) and item.get("status") == "fail"),
        },
        "operations": {
            "approved": approved,
            "rejected": rejected,
            "ops_actions_count": len(ops_actions),
        },
        "key_metrics": {
        ...
        "recommendations": [
            (action.model_dump(mode="json") if hasattr(action, "model_dump") else dict(action)).get("recommendation", "") 
            for action in ops_actions[:5]  # Top 5 recommendations
        ],
    }
    io.write_json_sorted(summary_payload_data, out_dir / "summary.json")
    
    # Generate sign_off.pdf (executive summary PDF)
    sign_off_pdf_path = out_dir / "sign_off.pdf"
    try:
        import subprocess
        import shutil
        
        # Try to generate PDF from markdown using pandoc
        pandoc_path = shutil.which("pandoc")
        if pandoc_path:
            # Create markdown content for sign-off
            md_content = f"""# Executive Summary - Business Validation Report
...
- **Total Terms:** {summary_payload_data['sla_summary']['total_terms']}
- **Passed:** {summary_payload_data['sla_summary']['pass']}
- **Warnings:** {summary_payload_data['sla_summary']['warn']}
- **Failed:** {summary_payload_data['sla_summary']['fail']}

## Operations

- **Approved Insights:** {approved}
- **Rejected Insights:** {rejected}
- **Operational Actions:** {summary_payload_data['operations']['ops_actions_count']}

## Key Recommendations

"""
            for i, rec in enumerate(summary_payload_data['recommendations'], 1):
                if rec:
                    md_content += f"{i}. {rec}\n"
            
            md_content += f"""
---
*This report was generated automatically by the Business Validation pipeline.*
"""
            
            # Write markdown temporarily
            md_path = out_dir / "sign_off.md"
            md_path.write_text(md_content, encoding="utf-8")
            
            # Convert to PDF
            cmd = [pandoc_path, str(md_path), "-o", str(sign_off_pdf_path)]
            ...
            if result.returncode == 0 and sign_off_pdf_path.exists():
                pass  # Success
            else:
                # If PDF generation fails, create empty file or copy markdown
                sign_off_pdf_path.write_bytes(b"")  # Empty PDF as fallback
        else:
            # If pandoc not available, create empty PDF file
            sign_off_pdf_path.write_bytes(b"")
    except Exception:
    ...
    outputs = {
        ...
        "what_if": (out_dir / "what_if.yaml").as_posix(),
        "sign_off": sign_off_pdf_path.as_posix(),
    }
