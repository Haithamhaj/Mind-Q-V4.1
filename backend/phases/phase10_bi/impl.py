...
def run(run_id: str, inputs: Optional[Mapping[str, Any]], config: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    insights_summary = insights_payload.get("summary") if isinstance(insights_payload, Mapping) else None
    insights_summary = insights_summary if isinstance(insights_summary, Mapping) else {}
    ...
    # Also generate metrics.json (expected output)
    metrics_json_path = semantic_dir / "metrics.json"
    metrics_json_path.write_text(json.dumps(semantic_payload, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    # Generate catalog.json (semantic catalog with artifact paths)
    # Note: dataset_path and insights_path will be defined below, but we prepare catalog payload here
    catalog_path = semantic_dir / "catalog.json"
    ...
    # Now generate catalog.json with all paths available
    catalog_payload = {
        "run_id": run_id,
        ...
        "marts": [
            {
                "id": mart["id"],
                "files": mart["files"],
                "path": (marts_dir / mart["files"][0]).as_posix() if mart["files"] else None,
            }
            for mart in semantic_marts
        ],
        "metrics": semantic_payload.get("metrics", []),
        "dimensions": semantic_payload.get("dimensions", []),
        "artifact_paths": {
            "metrics_yaml": _relative_path(metrics_yaml_path, stage10_dir),
            "metrics_json": _relative_path(metrics_json_path, stage10_dir),
            "dimensions": _relative_path(dimensions_path, stage10_dir),
            "insights": _relative_path(insights_path, stage10_dir),
            "orders_dataset": _relative_path(dataset_path, stage10_dir),
        },
    }
    catalog_path.write_text(json.dumps(catalog_payload, ensure_ascii=False, indent=2), encoding="utf-8")
    
    # Generate assistant/prompts.json (BI assistant prompts)
    assistant_dir = stage10_dir / "assistant"
    ...
    prompts_payload = {
        ...
        "timezone": timezone,
        "currency": currency,
        "available_marts": [mart["id"] for mart in semantic_marts],
        "available_metrics": [metric["id"] for metric in semantic_payload.get("metrics", [])],
        "available_dimensions": [dim["id"] for dim in semantic_payload.get("dimensions", [])],
        "system_prompt": (
            f"You are a BI assistant for a logistics analytics platform. "
            f"Run ID: {run_id}. Timezone: {timezone}. Currency: {currency}. "
            f"Available marts: {', '.join([mart['id'] for mart in semantic_marts])}. "
            f"Help users query and understand their logistics data."
        ),
        "example_queries": [
            "What is the total number of orders?",
            "Show me orders by destination",
            "What is the COD rate?",
            "Show me average COD amount by destination",
        ],
        "query_templates": {
            "total_orders": "SELECT COUNT(*) AS total FROM fact_business",
            "orders_by_destination": "SELECT DESTINATION, COUNT(*) AS count FROM fact_business GROUP BY DESTINATION ORDER BY count DESC LIMIT 20",
            "cod_rate": "SELECT AVG(CASE WHEN RECEIVER_MODE = 'COD' THEN 1.0 ELSE 0.0 END) * 100 AS cod_rate FROM fact_business",
        },
        "artifact_paths": {
            "metrics": _relative_path(metrics_json_path, stage10_dir),
            "catalog": _relative_path(catalog_path, stage10_dir),
            "dimensions": _relative_path(dimensions_path, stage10_dir),
        },
    }
    prompts_path = assistant_dir / "prompts.json"
    ...
    return {
        ...
        "outputs": {
            ...
            "semantic_metrics_yaml": metrics_yaml_path.as_posix(),
            "semantic_metrics_json": metrics_json_path.as_posix(),
            "semantic_catalog": catalog_path.as_posix(),
            "semantic_dimensions": dimensions_path.as_posix(),
            "insights_path": insights_path.as_posix(),
            "dataset_path": dataset_path.as_posix(),
            "assistant_prompts": prompts_path.as_posix(),
        },
        "metrics": {
