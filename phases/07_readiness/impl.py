...
def run(run_id: str, inputs: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out_dir = os.path.join(artifacts_root, run_id, "stage_07_readiness")
    ...
    try:
        ...
        validate.assert_row_stability(n_in=n_rows, n_out=n_rows, allow_drop=False, phase="07", out_dir=Path(out_dir))
    except SystemExit:
        pass
    with open(os.path.join(out_dir, "row_meta.json"), "w", encoding="utf-8") as f:
        json.dump({"phase": "07", "n_rows": n_rows, "source": raw}, f)
    return {
        "run_id": run_id,
        "status": "PASS",
        "outputs": {"raw": raw},
