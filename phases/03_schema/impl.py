...
def run(run_id: str, inputs: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out_dir = os.path.join(artifacts_root, run_id, "stage_03_schema")
    ...
    with open(os.path.join(out_dir, "row_meta.json"), "w", encoding="utf-8") as f:
        json.dump({"phase": "03", "n_rows": n_rows, "source": raw}, f)
    return {
        "run_id": run_id,
        "status": "PASS",
        "outputs": {"raw": raw, "schema": os.path.join(out_dir, "schema_v1.json")},
